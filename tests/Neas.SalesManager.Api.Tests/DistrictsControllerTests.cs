// tests/Neas.SalesManager.Api.Tests/DistrictsControllerTests.cs
...
public class DistrictsControllerTests
{
    ...
    [Fact]
    public async Task AssignSalesperson_ReturnsNoContent_WhenRequestIsValid()
    {
        // Arrange
        int districtId = 1;
        var request = new AssignSalespersonRequest(SalespersonId: 2, IsPrimary: true);

        _mockRepo.Setup(r => r.AssignSalespersonAsync(districtId, request.SalespersonId, request.IsPrimary))
        ...
        _mockRepo.Verify(r => r.AssignSalespersonAsync(districtId, 2, true), Times.Once);
    }
